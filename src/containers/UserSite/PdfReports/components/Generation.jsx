              ...
              {data && Object.keys(data).map((item, key) => (
                <Col key={key} md={6} lg={6} style={{ paddingTop: '20px' }}>
                  ...
                  {data[item].chartData.length > 0 ?
                    <ul style={{ listStyle: 'inside', textAlign: 'left', marginLeft: 20 }}><li style={{ width: '430px' }}>{organization} recycled {data[item].total.toFixed(2)} KG of recyclable waste in {item}.</li></ul>
                    :
                    <ul style={{ listStyle: 'inside', textAlign: 'center', marginLeft: 20 }}><li style={{ width: '430px' }}>There was no waste pickup in {item}.</li></ul>
                  }
                </Col>
              ))}
            </Row>
