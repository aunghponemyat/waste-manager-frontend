...
class ReportingForm extends React.Component {
  ...
  getUserDetail = () => {
    ...
    getUserDetailWithPromise(id)
      .then((response) => {
        console.log('User Detail Response:', response); // ← log response
        this.setState({ organization: response.organizationId._id });
      })
      .catch((err) => {
        console.error('Error fetching user detail:', err); // ← catch errors
      });
  }
  ...
  render() {
    ...
    return (
      <Col md={12} lg={12}>
        <Card>
          <CardBody className="report-container">
            {/* Hero Section */}
            <div
            ...
            >
              <h2 style={{ fontSize: '2rem', marginBottom: '20px' }}>Generate Report</h2>
              {organization && (
                <CreateQuarterModal
            ...
            <section className="report-section">
              <h4 className="report-title">About Report</h4>
              <p>
                Our Waste Management Report identifies, quantifies, and analyses the composition of the waste stream generated to ensure compliance with ISO 14001-2015, YCDC, and regional environmental standards in Southeast Asia.
              </p>
              <p>
                We visualize the collected weight and percentage of each waste type for easier understanding of waste trends and to support decision-making in office policy.
              </p>
              <p>
                Download quarterly reports in formats that include graphs, charts, and spreadsheets showing the quantity and quality of recyclable waste.
              </p>
              <p>
                We welcome your feedback. For suggestions or additional data requests, contact us at <a href="mailto:contact@recyglo.com">contact@recyglo.com</a> or <a href="tel:+959404245800">+95-9-40424-5800</a>.
              </p>
            </section>
            ...
            {JSON.stringify(data) !== '{}' && generationData && (
              <section className="report-section">
                <h4 className="report-title">Waste Generation Overview</h4>
                <Generation
                  data={generationData}
                  title={data.quarter}
            ...
            {trendlineData && (
              <section className="report-section">
                <h4 className="report-title">Trend Line</h4>
                <TrendLineGraph
                  data={trendlineData}
                  months={Object.keys(data.ways)}
                  organization={organization.name}
                />
              </section>
            )}
            ...
            {JSON.stringify(data) !== '{}' && (
              <section className="report-section">
                <h4 className="report-title">Items Found</h4>
                <ItemsFound
                  data={data}
                  reportDate={new Date()}
                />
            ...
            {JSON.stringify(data) !== '{}' && generationData && (
              <section className="report-section">
                <h4 className="report-title">Waste Composition</h4>
                <Composition
                  data={generationData}
                  title={data.quarter}
                />
            ...
            {JSON.stringify(data) !== '{}' && generationData && (
              <section className="report-section">
                <h4 className="report-title">Total Composition by Month</h4>
                <TotalComposition
                  months={Object.keys(data.ways)}
                  organization={organization.name}
                  data={totalCompositionData}
            ...
            {JSON.stringify(data) !== '{}' && findings && recommendations && (
              <section className="report-section">
                <h4 className="report-title">Summary & Recommendations</h4>
                <Summary
                  organization={organization.name}
